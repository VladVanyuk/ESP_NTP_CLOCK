...
void printWeatherLCD(bool forcePrint = false, bool setChars = false)
{
    ...
    if ((millis() - lastPrintedWeatherTime > timerSyncDelay_w) || forcePrint)
    {
        lastPrintedWeatherTime = millis();
        LCD.clear();

        int tempInt = (int)Weather.temp;
        float tempDecimal = Weather.temp - tempInt;

        bigNum.displayLargeInt(tempInt, 0, 0, 2, false);
        LCD.setCursor(10, 0);
        LCD.print((char)223);
        LCD.print('C');
        LCD.setCursor(0, 1);
        ...
        int humInt = (int)Weather.humidity;
        bigNum.displayLargeInt(humInt, 0, 0, 2, false);
        LCD.setCursor(10, 0);
        LCD.print('%');
        LCD.setCursor(0, 1);
        LCD.print("H:");
        LCD.print(Weather.humidity, 0);
        LCD.write((char)LCD_DROPLET);
